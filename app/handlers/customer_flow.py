...
class CustomerFlowHandler:
    ...
    @classmethod
    async def handle_message(cls, sender_phone: str, body: str, has_media: bool, media: Optional[Dict[str, Any]]) -> None:
        db = get_database()
        ...
        if state == UserState.IDLE.value:
            ...
            await cls._handle_id_card_submission(clean_phone, has_media, media)
    ...
    @classmethod
    async def _handle_id_card_submission(cls, phone: str, has_media: bool, media: Optional[Dict[str, Any]]):
        if not has_media or not media or not media.get("filePath"):
            await whatsapp_service.send_message(
                phone,
        ...
        ver_doc = await VerificationService.create_verification_request(phone, file_path)
