...
def format_admin_verification_alert(ver: Dict[str, Any], user_phone: str) -> str:
    return (
        ...
        f"• *ID:* `{ver.get('verification_id')}`\n"
        f"• *Student Phone:* `+{user_phone}`\n"
        f"• *Submitted:* {format_ist(ver.get('created_at'))}\n\n"
        f"Review the student ID card photo above and reply with:\n"
        f"👉 `/approve {ver.get('verification_id')}`\n"
        f"👉 `/reject {ver.get('verification_id')} <reason>`"
    )
